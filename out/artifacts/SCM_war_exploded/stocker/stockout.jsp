...
<c:forEach items="${list}" var="pr" varStatus="no">
            <tr >
                <td align="center"><a onclick="gotoDetail('${pr.SOID}')">${pr.SOID}</a></td>
                <td align="center">${pr.createTime}</td>
                <td align="center">${pr.account}</td>
                <td align="center">${pr.customerName}</td>
                ...
                <td align="center">
                    <a onclick="ChuKu('${pr.SOID}')" onmouseover="changeHand(this)" >出库</a>
                </td>
            </tr>
</c:forEach>
...
<script>
            ...
            success:function (result) {
                ...
                for(var i=0;i<result.length;i++){
                    table.append("<tr>"+
                        ...
                        "<td align=\"center\">" +
                        "<a href=\"#\" onclick='ChuKu("+re[i].a+")' onmouseover='changeHand(this)'>出库</a>" +
                        "</td>" +
                        "</tr>")
                }
    ...
    function ChuKu(soid) {
        var flag = confirm("确定出库吗？");
        if (flag){
            var url = "../stock/chukuservice";
            $.ajax({
                url: url,
                dataType: 'text',
                data:{"soid":soid},
